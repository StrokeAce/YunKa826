...
void CMainFrame::JsCallMFC(WPARAM wParam, LPARAM lParam)
{
	...
	else if (wParam == JS_CALL_RERECV_FILE)
	{
		...
		char url[MAX_1024_LEN];
		strcpy(url, reRecvParams->url.c_str());
		...
		m_manager->ReRecv_Msg(url, msgFromUserType, msgId.c_str(), msgDataType, msgFromUserId, assistUserId, 0);
	}
...
void CMainFrame::ShowMySelfSendMsg(string strMsg, MSG_DATA_TYPE msgType, string msgId)
{
	...
	char name[MAX_1024_LEN];
	char msg[MAX_1024_LEN];
	string imagePath;
	...
	f_covet.Gb2312ToUTF_8(head, headPath.c_str(), headPath.length());
...
void CMainFrame::InitControl()
{
	...
	showText.Format(L"%d", m_manager->GetSelfInfo()->UserInfo.uid);
	...
	showText.Format(L"账号    %d", m_manager->GetSelfInfo()->UserInfo.uid);
	showNameLabel = static_cast<CLabelUI*>(m_PaintManager.FindControl(_T("show_user_id")));
	if (showNameLabel)
		showNameLabel->SetText(showText.GetData());

	ANSIToUnicode(m_manager->GetSelfInfo()->UserInfo.nickname, buf);
...
void CMainFrame::ShowSysSettingWnd()
{
	...
	m_hSystemSettings->m_sysConfig = m_manager->GetSystemConfig();
...
void CMainFrame::SendMsgToGetList()
{
	...
	for (iter = m_manager->m_listEarlyChat.begin(); iter != m_manager->m_listEarlyChat.end(); iter++)
	{
		...
		if (id > 0)
		{
			...
			m_allVisitorNodeMap.insert(pair<unsigned long, UserListUI::Node*>(pWebUser->webuserid, addNode));
			m_allVisitorUserMap.insert(pair<unsigned long, unsigned long>(pWebUser->webuserid, GetSelfInfo()->UserInfo.uid));
...
void CMainFrame::OnManagerButtonEvent(TNotifyUI& msg)
{
	...
	{
		sprintf(sURL, m_manager->m_initConfig.webpage_querywebphone, m_manager->m_vip.c_str(), m_manager->GetSelfInfo()->UserInfo.uid, m_manager->m_login->m_szAuthtoken);
		ShowWebBrowser(sURL);
	}
...
void CMainFrame::LoadBrowser(char* url)
{
	if (m_pVisitorRelatedHandler.isCreated)
	{
		if (m_pVisitorRelatedHandler.handler->GetBrowser())
		{
			if (url == NULL)
			{
				m_pVisitorRelatedHandler.handler->LoadString(m_defaultUrlInfo);
			}
			else
			{
