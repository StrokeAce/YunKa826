	...
	CefRefPtr<CefMenuModel> model)
{
	...
	if ((params->GetTypeFlags() & (CM_TYPEFLAG_PAGE | CM_TYPEFLAG_FRAME)) != 0)
	{
			...
			((params->GetTypeFlags() & CM_TYPEFLAG_SELECTION) != 0))
		{
			...
			{
				string copy = "复制";
				char copyCvt[MAX_64_LEN];
				convert.Gb2312ToUTF_8(copyCvt, copy.c_str(), copy.length());
		...
		else
		{
			...
			{
				string reFresh = "刷新";
				char reFreshCvt[MAX_64_LEN];
				convert.Gb2312ToUTF_8(reFreshCvt, reFresh.c_str(), reFresh.length());
