...
CMainFrame::CMainFrame(IChatManager* manager) :m_manager(manager)
{
	//初始化
	...
	char defUrl[MAX_1024_LEN];
	convert.Gb2312ToUTF_8(defUrl, strTmp.c_str(), strTmp.length());
	m_defaultUrlInfo = defUrl;
...
LRESULT CMainFrame::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	else if (uMsg == WM_KEYDOWN) // 发送消息框的Ctrl+V消息	
	{
		...
		if (wParam == VK_RETURN)   //m_manager->GetSystemConfig()->m_nKeySendType == 0)
		{

			int state = ::GetKeyState(VK_CONTROL);

			if (m_manager->GetSystemConfig()->m_nKeySendType == 0 && ::GetKeyState(VK_CONTROL) >= 0)
			{
				if (m_pSendEdit != NULL && m_pSendEdit->IsFocused())
				{
			...
			if (m_manager->GetSystemConfig()->m_nKeySendType == 1 && ::GetKeyState(VK_CONTROL) < 0)
			{
				if (m_pSendEdit != NULL && m_pSendEdit->IsFocused())
				{
					OnBtnSendMessage(msg);
...
#if 0
	if (wParam == VK_RETURN  && ::GetKeyState(VK_CONTROL) > 0)   //m_manager->GetSystemConfig()->m_nKeySendType == 0)
	{

		if (m_manager->GetSystemConfig()->m_nKeySendType == 0)
		{
			if (m_pSendEdit != NULL && m_pSendEdit->IsFocused())
			{
				OnBtnSendMessage(msg);
		...
		else if (m_manager->GetSystemConfig()->m_nKeySendType == 1)
		{
			if (m_pSendEdit != NULL && m_pSendEdit->IsFocused())
			{
	...
	{
		if (m_pSendEdit != NULL && m_pSendEdit->IsFocused() && m_manager->GetSystemConfig()->m_nKeySendType == 1)
		{
			OnBtnSendMessage(msg);
		}
	}
	...
	if (m_manager->GetSystemConfig()->m_nKeySendType == 0 && (m_pSendEdit != NULL) && m_pSendEdit->IsFocused())
	OnBtnSendMessage(msg);
	}

	else  if (wParam == VK_RETURN && m_controlKeyState == 1)
	{
	if (m_manager->GetSystemConfig()->m_nKeySendType == 1 && (m_pSendEdit != NULL) && m_pSendEdit->IsFocused())
	OnBtnSendMessage(msg);
	}
	*/
...
void CMainFrame::InitLibcef(void)
{
	...
	if (!m_pListMsgHandler.isCreated)
	{
		...
		CCodeConvert convert;
		char utfUrl[MAX_1024_LEN];
		convert.Gb2312ToUTF_8(utfUrl, localUrl.c_str(), localUrl.length());
...
void CMainFrame::OnItemRbClick(TNotifyUI &msg)
{
	...
	if (msg.pSender->GetName() == L"userlist")
	{
		...
		CDuiString key;
		switch (m_manager->GetSystemConfig()->m_nFilterType)
		{
		...
		else if (uid == m_manager->GetSelfInfo()->UserInfo.uid) //自己的uid
		{
			xmlPath = L"menu\\menu_right_myself_node.xml";
		}
...
void CMainFrame::OnBtnSendMessage(TNotifyUI& msg)
{
	...
	if (sendUserType == MSG_RECV_CLIENT)
	{
		...
		{
			ShowOperationTips(_T("对方不在线，不能发送消息"));
			return;
		}
	}
	else if (sendUserType == MSG_RECV_WEB || sendUserType == MSG_RECV_WX)
	{
		...
		{
			ShowOperationTips(_T("会话已关闭，不能发送消息"));
			return;
		}
	}
	...
	if (sendMsgType == MSG_DATA_TYPE_TEXT)
	{
		...
		m_manager->SendTo_Msg(m_curSelectId, sendUserType, msgId.c_str(), MSG_DATA_TYPE_TEXT, sendMsgData.c_str());
	...
	else if (sendMsgType == MSG_DATA_TYPE_IMAGE)
	{
		...
		{
			m_manager->SendTo_Msg(m_curSelectId, sendUserType, msgId.c_str(), MSG_DATA_TYPE_IMAGE, vecName[i].c_str());
...
void CMainFrame::SendMsgToGetList()
{
	...
	//这里查找有没有 没有接入的对话 
	CCustomList* userList = m_manager->GetEarlyChatList();
	int length = 0;
	if (userList) length = userList->GetLength();
	
	for (int i = 0; i < length; i++)
	{
		unsigned long id = userList->GetAt(i);
		if (id > 0)
		{
			CWebUserObject *pWebUser = NULL;
			...
			m_allVisitorUserMap.insert(pair<unsigned long, unsigned long>(pWebUser->webuserid, m_manager->GetSelfInfo()->UserInfo.uid));
...
void CMainFrame::RecvMsg(IBaseObject* pObj, MSG_FROM_TYPE msgFrom, const char* msgId, MSG_TYPE msgType, MSG_DATA_TYPE msgDataType, const char* msgContent,
...
void CMainFrame::ResultRecvMsg(const char* msgId, bool bSuccess, const char* url, unsigned long msgFromUserId,
	unsigned long assistUserId, const char* filePath, MSG_FROM_TYPE msgFromType, MSG_DATA_TYPE msgDataType)
{
	...
	if (bSuccess)
	{
		sprintf(strCallJs, "ResultRecvMsg('%s','%d','%s','unknown','%s','%d','%d','%lu','%lu');",
			msgId, bSuccess, url, filePath, msgFromType, msgDataType, msgFromUserId, assistUserId);
	}
	else
	{
		CCodeConvert convert;
		char utfImagePath[MAX_1024_LEN];
		string imagePath = FullPath("SkinRes\\mainframe\\");
		StringReplace(imagePath, "\\", "/");
		convert.Gb2312ToUTF_8(utfImagePath, imagePath.c_str(), imagePath.length());
		sprintf(strCallJs, "ResultRecvMsg('%s','%d','%s','%s','%s','%d','%d','%lu','%lu');",
			msgId, bSuccess, url, utfImagePath, "",
...
void CMainFrame::ResultSendMsg(const char* msgId, bool bSuccess, unsigned long userId, MSG_RECV_TYPE recvUserType,
	MSG_DATA_TYPE msgDataType, const char* msg)
{
	...
	if (bSuccess)
	{
		if (msgDataType == MSG_DATA_TYPE_IMAGE || msgDataType == MSG_DATA_TYPE_VOICE)
		{
			sprintf(strCallJs, "ResultSendMsg('%s','%d','0','%s','%d','%d','%lu');",
				msgId, bSuccess, msg, recvUserType, msgDataType, userId);
		}
		else if (msgDataType == MSG_DATA_TYPE_FILE)
		{
			string sMsg = msg;
			int pos = sMsg.find_last_of("/");
			string fileName = sMsg.substr(pos + 1, sMsg.length() - pos - 1);
			sprintf(strCallJs, "ResultSendMsg('%s','%d','%s','%s','%d','%d','%lu');",
				msgId, bSuccess, fileName.c_str(), sMsg.c_str(), recvUserType, msgDataType, userId);
		}
	}
	else
	{
		...
		StringReplace(imagePath, "\\", "/");
		convert.Gb2312ToUTF_8(utfImagePath, imagePath.c_str(), imagePath.length());
		sprintf(strCallJs, "ResultSendMsg('%s','%d','%s','%s','%d','%d','%lu');", msgId, bSuccess,
			utfImagePath, msg, recvUserType, msgDataType, userId);
	}
...
void CMainFrame::ResultScreenCapture(const char* imagePath)
{
...
void CMainFrame::OnManagerButtonEvent(TNotifyUI& msg)
{
	...
	else if (msg.pSender->GetName() == _T("managerbutton_3"))
	{
		...
		sprintf(sURL, m_manager->GetConfig()->webpage_SvrMsg, m_manager->GetVip(), m_manager->GetAuthToken());
		ShowWebBrowser(sURL);
	}
	...
	{
		sprintf(sURL, m_manager->GetConfig()->webpage_querywebphone, m_manager->GetVip(), m_manager->GetSelfInfo()->UserInfo.uid, m_manager->GetAuthToken());
		ShowWebBrowser(sURL);
	}
	...
	{
		sprintf(sURL, m_manager->GetConfig()->webpage_note, m_manager->GetVip(), m_manager->GetSelfInfo()->UserInfo.uid, m_manager->GetAuthToken());
		ShowWebBrowser(sURL);
	}
	...
	else if (msg.pSender->GetName() == _T("managerbutton_6"))
	{
		...
		sprintf(sURL, m_manager->GetConfig()->webpage_crm, m_manager->GetVip(), m_manager->GetAuthToken());
		ShowWebBrowser(sURL);
	}
	...
	else if (msg.pSender->GetName() == _T("managerbutton_7"))
	{
		...
		sprintf(sURL, m_manager->GetConfig()->webpage_workbillurl, m_manager->GetVip(), m_manager->GetAuthToken());
		ShowWebBrowser(sURL);
	}
	...
	else if (msg.pSender->GetName() == _T("managerbutton_8"))
	{
		...
		sprintf(sURL, m_manager->GetConfig()->webpage_mgmt, m_manager->GetVip(), m_manager->GetAuthToken());
		ShowWebBrowser(sURL);
	}
...
void CMainFrame::ShowMySelfSendMsg(string strMsg, MSG_DATA_TYPE msgType, string msgId)
{
	unsigned long userId = 0;
	CCodeConvert convert;
	char strJsCode[MAX_2048_LEN] = { 0 };
	char name[MAX_USERNAME_LEN + 1] = { 0 };
	char msg[MAX_1800_LEN] = { 0 };
	string imagePath;

	if (strMsg.empty() || strMsg.length() > MAX_1800_LEN)
	{
		//g_WriteLog.WriteLog(C_LOG_ERROR, "插入空的聊天记录");
		return;
	}

	string strName = m_manager->GetSelfInfo()->UserInfo.nickname;
	StringReplace(strName, "'", "&#039;");
	StringReplace(strName, "\r\n", "<br>");
	convert.Gb2312ToUTF_8(name, strName.c_str(), strName.length());
	...
	{
		sprintf(msg, "<img id=\"%s_image\" class=\"wait_image\" src=\"%s\"><audio class=\"msg_voice\" controls=\"controls\" src=\"%s.wav\" type=\"audio/mpeg\"></audio>",
	...
	//组合消息
	string headPath;
	char head[MAX_1024_LEN];
	headPath = "<img id=\"" + msgId + "_head\" class=\"head_image\" src=\"" + m_manager->GetSelfInfo()->m_headPath + "\">";
	convert.Gb2312ToUTF_8(head, headPath.c_str(), headPath.length());

	sprintf(strJsCode, "AppendMsgToHistory('%d', '%d', '%s', '%s', '%s', '%lu', '%s', '%s', '%s'); ",
		MSG_FROM_SELF, msgType, name, GetTimeByMDAndHMS(0).c_str(), msg, userId, head, msgId.c_str(), imagePath.c_str());
...
void CMainFrame::ShowRightOptionFrameView(unsigned long id, string sid)
{
	...
	try
	{
		...
		case TYPESELECT_INFO:
			if (pWebUser->m_bIsGetInfo)
			{
				if (pWebUser->m_bIsFrWX)
				{
					...
					convert.Gb2312ToUTF_8(msg, content.c_str(), content.length());
				}
				else
				{
					// web用户信息
					convert.Gb2312ToUTF_8(msg, pWebUser->m_strInfoHtml.c_str(), pWebUser->m_strInfoHtml.length());
				}
		...
		case TYPESELECT_CLIENT:
			...
			sprintf(showMsg, m_manager->GetConfig()->visitorpage_visitortail, m_manager->GetVip(), pWebUser->chatid, pWebUser->webuserid, pWebUser->info.sid, 0, nTransFrom, strFrom.c_str(), strEnd.c_str(), m_manager->GetSelfInfo()->UserInfo.uid);
			strUrl = showMsg;
			break;
		case TYPESELECT_CHATID:
			sprintf(showMsg, m_manager->GetConfig()->visitorpage_visitorbill, m_manager->GetVip(), pWebUser->chatid, pWebUser->webuserid, pWebUser->info.sid, 0, 0, strFrom.c_str(), strEnd.c_str());
			strUrl = showMsg;
			break;
		case TYPESELECT_CLIENTINFO:
			//客户信息
			sprintf(showMsg, m_manager->GetConfig()->visitorpage_visitorinfo, m_manager->GetVip(), pWebUser->chatid, pWebUser->webuserid, pWebUser->info.sid, 0, 0, strFrom.c_str(), strEnd.c_str(), pWebUser->info.sid);
			strUrl = showMsg;
			break;
		case TYPESELECT_NOTICE:
			...
			if (pWebUser->m_bIsFrWX)
			{
				sprintf(showMsg, m_manager->GetConfig()->visitorpage_visitororder, m_manager->GetVip(), pWebUser->chatid, pWebUser->webuserid, pWebUser->info.sid, 0, 0, strFrom.c_str(), strEnd.c_str(), pWebUser->info.thirdid, pWebUser->m_sWxAppid, 0);
				strUrl = showMsg;
			}
			else
			{
				...
				convert.Gb2312ToUTF_8(msg, strTmp.c_str(), strTmp.length());
				m_pVisitorRelatedHandler.handler->LoadString(msg);
			}
			break;
		...
		case TYPESELECT_ORDER:
			//查订单
			//strURL.Format(pApp->GetConfig()->visitorpage_visitororder, pWebUser->chatid, pWebUser->webuserid, pWebUser->info.sid, 0, 0, strFrom, strEnd, pWebUser->info.thirdid, pWebUser->m_sWxAppid, 1);
			sprintf(showMsg, m_manager->GetConfig()->visitorpage_visitororder, m_manager->GetVip(), pWebUser->chatid, pWebUser->webuserid, pWebUser->info.sid, 0, 0, strFrom.c_str(), strEnd.c_str(), pWebUser->info.thirdid, pWebUser->m_sWxAppid, 1);
			strUrl = showMsg;
...
void CMainFrame::OnMenuEvent(CDuiString controlName)
{
	...
	{
		if (m_manager->GetSystemConfig()->m_nKeySendType == 1)
			m_manager->GetSystemConfig()->m_nKeySendType = 0;
	}
	else if (controlName == L"MenuElement_btn_send_menu_ctrl_enter") //按Ctrl + enter 键发送
	{
		if (m_manager->GetSystemConfig()->m_nKeySendType == 0)
			m_manager->GetSystemConfig()->m_nKeySendType = 1;
	}
	...
	else if (controlName == L"menu_sys_set")
	{
		//if (m_pSystSettingsDlg == NULL)
		{
			...
			m_pSystSettingsDlg->m_sysConfig = m_manager->GetSystemConfig();
	...
	{
		m_manager->GetSystemConfig()->m_nFilterType = VISITORFILTER_ALL;
	}

	//显示我的全部联系人
	else if (controlName == L"MenuElement_right_my_history_visitor")
	{
		m_manager->GetSystemConfig()->m_nFilterType = VISITORFILTER_MYVISITOR;
	}

	//只显示全部最近联系人
	else if (controlName == L"MenuElement_right_all_history_visitor")
	{
		m_manager->GetSystemConfig()->m_nFilterType = VISITORFILTER_ALLVISITOR;
	}

	//只显示访问时间超过1分钟的联系人
	else if (controlName == L"MenuElement_right_1_minute")
	{
		m_manager->GetSystemConfig()->m_nFilterType = VISITORFILTER_1MINUTES;
	}
	//只显示访问时间超过3分钟的联系人    
	else if (controlName == L"MenuElement_right_3_minute")
	{
		m_manager->GetSystemConfig()->m_nFilterType = VISITORFILTER_3MINUTES;
	}

	//只显示访问时间超过5分钟的联系人
	else if (controlName == L"MenuElement_right_5_minute")
	{
		m_manager->GetSystemConfig()->m_nFilterType = VISITORFILTER_5MINUTES;
	}

	//只显示访问时间超过10分钟的联系人
	else if (controlName == L"MenuElement_right_10_minute")
	{
		m_manager->GetSystemConfig()->m_nFilterType = VISITORFILTER_10MINUTES;
	}
	...
	if (pWxUser != NULL)
	{
		...
		if (!pWxUser->nickname.empty())
		{
			if (pWxUser->LanguageType == LANGUAGE_UTF8)
			{
				...
				convert.UTF_8ToAnsi(getInfo, pWxUser->nickname.c_str(), pWxUser->nickname.length());
				sprintf(htmlitem, itemFormat, "昵称", getInfo);
		...
		if (!pWxUser->country.empty())
		{
			if (pWxUser->LanguageType == LANGUAGE_UTF8)
			{
				convert.UTF_8ToAnsi(getInfo, pWxUser->country.c_str(), pWxUser->country.length());
				sprintf(htmlitem, itemFormat, "国家", getInfo);
				htmlContent += htmlitem;
			}
			else
			{
		...
		if (!pWxUser->province.empty())
		{
			if (pWxUser->LanguageType == LANGUAGE_UTF8)
			{
				convert.UTF_8ToAnsi(getInfo, pWxUser->province.c_str(), pWxUser->province.length());
				sprintf(htmlitem, itemFormat, "省份", getInfo);
				htmlContent += htmlitem;
			}
			else
			{
		...
		if (!pWxUser->city.empty())
		{
			if (pWxUser->LanguageType == LANGUAGE_UTF8)
			{
				convert.UTF_8ToAnsi(getInfo, pWxUser->city.c_str(), pWxUser->city.length());
				sprintf(htmlitem, itemFormat, "城市", getInfo);
				htmlContent += htmlitem;
			}
			else
			{
...
void CMainFrame::OnBtnVoice(TNotifyUI& msg)
{
	...
	{
		// 提示不能给该类型用户发语音消息
		ShowOperationTips(_T("只能对微信用户发送语音消息"));
	}
	else
	{
...
void CMainFrame::JsCallMFC(WPARAM wParam, LPARAM lParam)
{
	...
	else if (wParam == JS_CALL_START_RECORD)
	{
		...
		if (result == CODE_AUDIO_IS_RECORDING)
		{
			ShowOperationTips(_T("同时只能录一段语音"));
		}
		else if (result == CODE_AUDIO_LITTLE_TIME)
		{
			ShowOperationTips(_T("录音时间太短"));
		}
		else if (result == CODE_AUDIO_NO_DEVICE)
		{
			ShowOperationTips(_T("操作系统录音设备未开启"));
		}
		else if (result == CODE_AUDIO_FAIL)
		{
			ShowOperationTips(_T("未知原因导致录音失败"));
		}
	...
	else if (wParam == JS_CALL_SEND_AUDIO)
	{
		...
		// 停止录制,并保存文件
		if (code == CODE_AUDIO_LITTLE_TIME)
		{
			ShowOperationTips(_T("录音时间太短"));
			return;
		}
		...
		m_manager->SendTo_Msg(userId, MSG_RECV_WX, msgId.c_str(), MSG_DATA_TYPE_VOICE, m_audioPath.c_str());
	}
	...
	else if (wParam == JS_CALL_RESEND_FILE)
	{
		...
		m_manager->SendTo_Msg(userId, recvUserType, msgId.c_str(), msgDataType, filaPath.c_str());
	}
	else if (wParam == JS_CALL_RERECV_FILE)
	{
		...
		m_manager->ReRecv_Msg(url.c_str(), msgFromUserType, msgId.c_str(), msgDataType, msgFromUserId, assistUserId, 0);
	}
...
void CMainFrame::RecvWebUserInfo(CWebUserObject* pWebUser, WEBUSER_INFO_NOTIFY_TYPE type)
{
	...
	else if (type == NOTIFY_ID) //更新webuerid
	{
		...
		if (pWebUser->onlineinfo.talkstatus == TALK_STATUS_TALK)
		{
			...
			fatherId = m_manager->GetSelfInfo()->UserInfo.uid;
...
//获取在线 坐席
void CMainFrame::RecvOnlineUsers(const char* pUsersXml)
{
	// ?? 内存泄漏pGroup
	CMarkupXml xml(pUsersXml);
	CGroupObject *pGroup = new CGroupObject();
	if (!ParseGroupUser(xml, pGroup, "Group", "Person"))
		return;
	...
	if (state == 1)
	{
		...
		if (m_selectUserId == m_manager->GetSelfInfo()->UserInfo.uid)
		{
		...
		else if (m_topWndType == 3)  //内部对话
		{
			...
			else
			{
				...
				m_allVisitorUserMap.insert(pair<unsigned long, unsigned long>(m_selectUserId, m_manager->GetSelfInfo()->UserInfo.uid));
...
// 坐席下线消息
void CMainFrame::RecvOffline(IBaseObject* pObj, bool bShow)
{
	if (pObj->m_nEMObType == OBJECT_USER)
	{
		CUserObject* pUser = (CUserObject*)pObj;
...
void CMainFrame::RecvChatInfo(CWebUserObject* pWebUser, CUserObject* pUser)
{
	...
	else if (pWebUser->onlineinfo.talkstatus == TALK_STATUS_TALK)
	{
		...
		//如果是自己的id 则加到自己下面
		if (pUser->UserInfo.uid == m_manager->GetSelfInfo()->UserInfo.uid)
		{
			UserListUI::Node* tempNode = pMySelfeNode->child(0);
			UserListUI::Node* addNode = pUserList->AddNode(text, pWebUser->webuserid, pWebUser->info.sid, tempNode);
			pUserList->ExpandNode(tempNode, true);
...
VISITOR_TYPE  CMainFrame::CheckIdForTalkType(unsigned long id)
{
	...
	{
		//先查找是不是 自己
		if (id == m_manager->GetSelfInfo()->UserInfo.uid)
	...
	else if (pWebUser->onlineinfo.talkstatus == TALK_STATUS_TALK)
	{
		...
		if (id == m_manager->GetSelfInfo()->UserInfo.uid) //自己底下
		{
...
TREENODEENUM  CMainFrame::CheckIdForNodeType(unsigned long id)
{
	...
	//是自己的孩子 
	if (fatherId == m_manager->GetSelfInfo()->UserInfo.uid)
	{
		for (int i = 0; i < 4; i++)
		{
...
void CMainFrame::RecvQuickReply(const char* quickReply)
{
	...
	//解析xml
	CMarkupXml   xml((char*)quickReply);
...
void CMainFrame::PopTrayTips(const char* strPopTips, const char* strTitle)
{
	...
	{
		_snprintf(icondata.szInfoTitle, sizeof(icondata.szInfoTitle) - 1, "%s 新消息", m_manager->GetConfig()->sProgrameNameMini);
	}
	else
	{
		_snprintf(icondata.szInfoTitle, sizeof(icondata.szInfoTitle) - 1, "%s %s", m_manager->GetConfig()->sProgrameNameMini, strTitle);
	}
...
bool CMainFrame::ParseGroupUser(CMarkupXml &xml, CGroupObject *pGroupOb, char *sGroupKey, char *sUserKey)
{
	if (pGroupOb == NULL)
		return false;
