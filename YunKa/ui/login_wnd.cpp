...
CLoginWnd::CLoginWnd()
{
	...
	static CManagerFactory* fac = NULL;
	fac = CManagerFactory::GetInstance();
	m_manager = fac->GetManager();
...
void CLoginWnd::StartLogin(string loginName, string password, bool isAutoLogin, bool isKeepPwd)
{
	...
	m_manager->StartLogin(loginName.c_str(), password.c_str(), isAutoLogin, isKeepPwd);
