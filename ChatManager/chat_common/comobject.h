...
class DLL_EXPORT CUserObject : public IBaseObject
{
public:
	CUserObject();
	~CUserObject();
...
class DLL_EXPORT CWebUserObject : public IBaseObject
{
public:
	CWebUserObject();
	~CWebUserObject();
...
// 组对象
class DLL_EXPORT CGroupObject : public IBaseObject
{
public:
	CGroupObject();
...
// 简易的vector，可按下表访问
class DLL_EXPORT CCustomList
{
public:
	CCustomList();
	~CCustomList();
	int GetLength();
	void PushBack(unsigned long val);
	unsigned long GetAt(int index);
	void Clear();

private:
	int mpr_size;
	int mpr_capacity;
	unsigned long* mpr_head;
};
