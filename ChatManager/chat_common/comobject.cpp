...
CUserObject::CUserObject()
{
	...
	char headPath[MAX_1024_LEN];
	convert.Gb2312ToUTF_8(headPath, strPath.c_str(), strPath.length());
	m_headPath = headPath;
}
...
CWebUserObject::~CWebUserObject()
{
	...
	if (m_pWxUserInfo) delete m_pWxUserInfo;
	m_strMsgs.clear();

	MapWebUserFLag::iterator iter;
	for (iter = m_mapUrlAndScriptFlagOb.begin(); iter != m_mapUrlAndScriptFlagOb.end(); iter++)
	{
...
CCustomList::CCustomList()
{
	...
	mpr_capacity = 50;
	mpr_size = 0;
	mpr_head = (unsigned long*)malloc(sizeof(int)* 50);
}
...
int CCustomList::GetLength()
{
	return mpr_size;
}

void CCustomList::PushBack(unsigned long val)
{
	if (mpr_size >= mpr_capacity)
	{
		unsigned long* old = mpr_head;
		mpr_head = (unsigned long*)malloc(sizeof(unsigned long)* mpr_capacity * 2);
		memcpy(mpr_head, old, sizeof(unsigned long)* mpr_capacity);
		mpr_capacity *= 2;
		free(old);
		old = nullptr;
	}
	*(mpr_head + mpr_size) = val;
	mpr_size++;
}

unsigned long CCustomList::GetAt(int index)
{
	if (mpr_size <= 0)
	{
		return 0;
	}
	return *(mpr_head + index);
}
