...
#include "md5/md5.h"
#include "http_load/switch_sock.h"
...
CChatManager::CChatManager()
{
	...
	m_listEarlyChat = new CCustomList();
	LoadINIResource();
	ReadSystemConfig();
...
bool CChatManager::StartLogin(const char* loginName, const char* password, bool isAutoLogin, bool isKeepPwd)
{
	// 登录之前就应当将消息回调接口设置好
	assert(m_handlerLogin != nullptr);
	assert(password != nullptr);
	assert(loginName != nullptr);

	if (m_handlerLogin)
	{
		if (password != nullptr && loginName != nullptr)
		{
			m_sLogin = loginName;
			m_password = password;
...
int CChatManager::RecvSrvRepUserinfo(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	if (!m_bLoginSuccess)
	{
		...
		sprintf(strJsCode, "AppendMsgToHistory('%d','%d','%s','%s','%s','%s','%s','%s');",
			MSG_FROM_SYS, MSG_DATA_TYPE_TEXT, "", GetTimeByMDAndHMS(0).c_str(), sMsg,
	...
	if (RecvInfo.uin > WEBUSER_UIN)
	{
		...
		if (pWebUser != NULL)
		{
			...
			if (pWebUser->m_bConnected)
			{
				...
				if (pWebUser->IsDisplay(m_sysConfig, m_userInfo.UserInfo.uid))
				...
				m_handlerMsgs->RecvOffline(pWebUser, pWebUser->m_bIsShow);
			}
		}
	}
	else
...
int CChatManager::RecvFloatTransFailed(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	//这里需要判断是不是要继续显示该用户
	if (pWebUser->m_bNotResponseUser || pInviteUser != NULL || pInviteUser == NULL)
	{
		pWebUser->cTalkedSatus = HASTALKED;
...
int CChatManager::RecvFloatCloseChat(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	else
	{
		...
		if (!pWebUser->m_bIsFrWX && pWebUser->IsOnline())
		{
			// 访客如果在线，退回到访问中节点
			pWebUser->m_nWaitTimer = -20;
			pWebUser->onlineinfo.talkstatus = TALK_STATUS_NO;
...
const char* CChatManager::GetMsgId()
{
	sprintf(m_sMsgId, "id_%d", m_nMsgId);
	m_idLock.Lock();
	m_nMsgId++;
	m_idLock.Unlock();
	return m_sMsgId;
	...
	if (GetContentBetweenString(msg, "rand=", "&", sid))
	{
			...
			&& m_userInfo.UserInfo.uid == recvuid && !pWebUser->m_bNewComm)//非等待应答的会话
		{
			...
			else
			{
				...
				m_listEarlyChat->PushBack(pWebUser->webuserid);
			}
		}
	}
}
...
CWebUserObject * CChatManager::GetWebUserObjectByScriptFlag(char *scriptflag)
{
	...
	MapWebUsers::iterator iter;
	for (iter = m_mapWebUsers.begin(); iter != m_mapWebUsers.end(); iter++)
	{
		pWebUser = iter->second;
		if (pWebUser && iter->second->ScriptFlagIsExist(scriptflag))
		{
			return pWebUser;
		}
	}
...
int CChatManager::SendTo_Msg(unsigned long userId, MSG_RECV_TYPE userType, const char* msgId, MSG_DATA_TYPE msgDataType, const char* msg)
{
	int nError = SYS_FAIL;	
	if (userId <= 0 || msgId == nullptr || msg == nullptr)
	{
		g_WriteLog.WriteLog(C_LOG_ERROR, "SendTo_Msg params error");
		m_lastError = "userId,msgId,msg都不能为空";
		return nError;
	}

	if ((int)strlen(msg) > MAX_1800_LEN)
	{
		g_WriteLog.WriteLog(C_LOG_ERROR, "SendTo_Msg msg > 1800");
		m_lastError = "发送的消息字数不能超过1800";
		return nError;
	}
...
const char* CChatManager::GetLastError()
{
	return m_lastError.c_str();
}

int CChatManager::ReRecv_Msg(const char* url, MSG_FROM_TYPE msgFromUserType, const char* msgId, MSG_DATA_TYPE nMsgDataType,
...
int CChatManager::SendTo_InviteWebUser(CWebUserObject *pWebUser, int type, const char* strText)
{
	int nError = SYS_FAIL;
	if (pWebUser != NULL)
	{
		...
		if (strText != nullptr)
			m_sysConfig->m_sInviteWords = strText;
		...
		sprintf(sbuff, "<MSG><COMMAND>%s</COMMAND><NEEDNOTE>NO</NEEDNOTE><TEXT>%s</TEXT><SERVICEUIN>%d</SERVICEUIN><SCRIPTFLAG>%s</SCRIPTFLAG><IP>%s</IP><PORT>%d</PORT><AUTO>0</AUTO></MSG>\r\n",
			GetApplyTypeString(type).c_str(), m_sysConfig->m_sInviteWords.c_str(), m_userInfo.UserInfo.uid, strScriptFlag.c_str(), pWebUser->info.sip, 2450);
...
void CChatManager::AfterUpload(unsigned long userId, MSG_RECV_TYPE userType, string msgId, string mediaID, MSG_DATA_TYPE msgDataType, string fileId, string filePath, string wxToken)
{
	...
	{
		// 上传失败处理,通知界面，文件发送失败
		m_handlerMsgs->ResultSendMsg(msgId.c_str(), false, userId, userType, msgDataType, filePath.c_str());
		...
		AddMsgToList(pObj, MSG_FROM_SELF, userType, msgId, MSG_TYPE_NORMAL,
			msgDataType, filePath.c_str(), 0, NULL, NULL, true, false, false);
		...
		if (msgDataType == MSG_DATA_TYPE_IMAGE)
		{
			if (userType == MSG_RECV_WX)
			{
				if (!fileId.empty())
				{
					...
					{
						m_handlerMsgs->ResultSendMsg(msgId.c_str());

						AddMsgToList((IBaseObject*)pWebUser, MSG_FROM_SELF, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL,
							msgDataType, filePath.c_str(), 0, NULL, NULL, true, false);
					}
				}
				else
				{
	...
	WxMsgBase* msgContentWx, bool bSave, bool bNotify, bool bAgain)
{
	ONE_MSG_INFO ongMsg;
	ongMsg.msgId = msgId;
	char head[MAX_1024_LEN] = "unKnow";
	char name[MAX_USERNAME_LEN + 1] = "unKnown";
	string sName;
	char sMsg[MAX_1800_LEN];
	char callJsMsg[MAX_2048_LEN];
	CCodeConvert convert;

	if (msgContent.length() > MAX_1800_LEN)
		return;
	...
	if (msgType == MSG_TYPE_NORMAL)
	{
		...
		if (pObj->m_nEMObType == OBJECT_WEBUSER)
		{
			...
			if (msgFrom == MSG_FROM_WEBUSER)
			{
				...
				{
					sprintf(head, "<img class=\"head_image\" src=\"%s\">",pWebUser->m_pWxUserInfo->headimgurl.c_str());
				}
				else
				{
					string defaultHead = FullPath("res\\headimage\\default.png");
					StringReplace(defaultHead, "\\", "/");
					defaultHead = "<img id=\"" + ongMsg.msgId + "_head\" class=\"head_image\" src=\"" + defaultHead + "\">";
					convert.Gb2312ToUTF_8(head, defaultHead.c_str(), defaultHead.length());
				}
				strcpy(name, pWebUser->info.name);
			}
			else if (msgFrom == MSG_FROM_ASSIST)
			{
				sprintf(head, "<img class=\"head_image\" src=\"%s\">", pAssistUser->m_headPath.c_str());
				strcpy(name, pAssistUser->UserInfo.nickname);
			}
			else if (msgFrom == MSG_FROM_SELF)
			{
				sprintf(head, "<img class=\"head_image\" src=\"%s\">", m_userInfo.m_headPath.c_str());
				strcpy(name, m_userInfo.UserInfo.nickname);
			}

			sName = name;
			StringReplace(sName, "'", "&#039;");
			convert.Gb2312ToUTF_8(name, sName.c_str(), sName.length());
			...
			if (bAgain)
			{
				...
				{
					sprintf(sMsg, "<img class=\"msg_image\" src=\"%s\" onclick=window.RunMsgList(\"ViewDetails\",\"%s\",\"2\")>", msgContent.c_str(), msgContent.c_str());
				}
				else if (msgDataType == MSG_DATA_TYPE_VOICE)
				{
					sprintf(sMsg, "<audio controls=\"controls\" class=\"msg_voice\" src=\"%s\" type = \"audio/mpeg\"></audio>", msgContent.c_str());
				}
				else if (msgDataType == MSG_DATA_TYPE_VIDEO)
				{
					sprintf(sMsg, "<video controls=\"controls\" class=\"msg_voice\" src=\"%s\" type = \"video/mp4\"></video>", msgContent.c_str());
				}
				...
				{
					sprintf(sMsg, "<span class=\"file_text\">链接：</span><a href=\"%s\" target=\"blank\">%s</a>",
			...
			sprintf(callJsMsg, "AppendMsgToHistory('%d','%d','%s','%s','%s','%s','%s','%s','%s');",
				msgFrom, msgDataType, name, GetTimeByMDAndHMS(msgTime).c_str(), sMsg, "0",
				head, ongMsg.msgId.c_str(), "");
		...
		else if (pObj->m_nEMObType == OBJECT_USER)
		{
			...
			{
				sprintf(head, "<img class=\"head_image\" src=\"%s\">", pUser->m_headPath.c_str());
				strcpy(name, pUser->UserInfo.nickname);
			}
			else if (msgFrom == MSG_FROM_SELF)
			{
				sprintf(head, "<img class=\"head_image\" src=\"%s\">", m_userInfo.m_headPath.c_str());
			...
			sprintf(callJsMsg, "AppendMsgToHistory('%d','%d','%s','%s','%s','%s','%s','%s');",
				msgFrom, msgDataType, name, GetTimeByMDAndHMS(msgTime).c_str(), sMsg, "0", head, ongMsg.msgId.c_str());
			ongMsg.msg = callJsMsg;
	...
	{
		m_handlerMsgs->RecvMsg(pObj, msgFrom, msgId.c_str(), msgType, msgDataType,
			callJsMsg, GetTimeByMDAndHMS(msgTime).c_str(), pAssistUser, msgContentWx);
	}
}
...
void CChatManager::RestartSession(LPARAM lParam)
{
	...
	CCodeConvert convert;
	char newUrl[MAX_1024_LEN];
	convert.Gb2312ToUTF_8(newUrl, strUrl.c_str(), strUrl.length());

	//发起二次会话 
	RepickChatCon(newUrl, strRet, nUin, strErrMsg);
}
...
void CChatManager::GetOnlineUser()
{
	...
	if (httpLoad.HttpLoad(string(strURL), "", REQUEST_TYPE_GET, "", strHtml))
	{
		m_handlerMsgs->RecvOnlineUsers(strHtml.c_str());
		return;
	}
	m_handlerMsgs->RecvOnlineUsers(nullptr);
}
...
UINT WINAPI CChatManager::DownLoadFileFromServerThread(void * para)
{
	...
	if (httpLoad.HttpLoad(url, "", REQUEST_TYPE_GET, downPath, rs))
	{
		...
		manager->m_handlerMsgs->ResultRecvMsg(msgId.c_str(), true, url.c_str(), webUserId, userId, addPath.c_str(), msgFrom, msgDataType);

		// 保存该条记录
		manager->AddMsgToList(pObj, msgFrom, MSG_RECV_ERROR, msgId, MSG_TYPE_NORMAL, msgDataType,
			addPath.c_str(), time, pUser, NULL, true, false);
		return true;
	}
	else
	{
		g_WriteLog.WriteLog(C_LOG_TRACE, "DownLoadFileFromServerThread error : %s", rs.c_str());
		manager->m_handlerMsgs->ResultRecvMsg(msgId.c_str(), false, url.c_str(), webUserId, userId, NULL, msgFrom, msgDataType);
		...
		manager->AddMsgToList(pObj, msgFrom, MSG_RECV_ERROR, msgId.c_str(), MSG_TYPE_NORMAL, msgDataType,
...
DWORD WINAPI CChatManager::GetQuickReplyThread(void *arg)
{
	...
	{
		pThis->m_handlerMsgs->RecvQuickReply(returnCode.c_str());
	}
...
void CChatManager::SolveAlertInfo(ALERT_TYPE type, string strPopTips)
{
	...
	if (pAlert->bTray)
	{
		m_handlerMsgs->PopTrayTips(strPopTips.c_str(), strTitle.c_str());
	}
...
CSysConfigFile* CChatManager::GetSystemConfig()
{
	return m_sysConfig;
}

CUserObject* CChatManager::GetSelfInfo()
{
	return &m_userInfo;
}

CCustomList* CChatManager::GetEarlyChatList()
{
	return m_listEarlyChat;
}

INIT_CONF* CChatManager::GetConfig()
{
	return &m_initConfig;
}

const char* CChatManager::GetVip()
{
	return m_vip.c_str();
}

const char* CChatManager::GetAuthToken()
{
	return m_login->m_szAuthtoken;
}

void CChatManager::SetLoginProgress(int percent)
{
	if (m_handlerLogin)
	{
		m_handlerLogin->LoginProgress(percent);
	}

	if (m_timers && percent == -1)
	{
		m_timers->KillTimer(TIMER_LOGIN);
	}
}
